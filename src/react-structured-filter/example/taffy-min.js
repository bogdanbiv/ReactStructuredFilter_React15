  ...
  if (!TAFFY) {
    ...
    s = function(z) {
      ...
      for (var y in z) {
        x[y] = k(z[y])
          ? z[y].toString()
          : T.isArray(z[y]) || T.isObject(z[y]) ? s(z[y]) : z[y];
      }
      return x;
    };
    a = function(y) {
    ...
    c = function(B, A, C) {
      ...
      if (B && ((T.isArray(B) && B.length === 1) || !T.isArray(B))) {
        ...
        for (
          E, D, z = 0, B = T.isArray(B) ? B : [B], F = B.length;
          z < F;
          z++
        ) {
          D = B[z];
            ...
            x =
              A.length === 1
                ...
                : A.length === 2
                  ? h(z, A[0]) || h(z, A[1])
                  : A.length === 3
                    ? h(z, A[0]) || h(z, A[1]) || h(z, A[2])
                    : A.length === 4
                      ? h(z, A[0]) || h(z, A[1]) || h(z, A[2]) || h(z, A[3])
                      : false;
            if (A.length > 4) {
              c(A, function(B) {
                if (h(z, B)) {
      ...
      if (T.isObject(y)) {
                        ...
                        : C === 'lte' || C === S
                          ...
                          : C === 'gte' || C === P
                            ...
                            : C === 'left'
                              ? N.indexOf(G) === 0
                              : C === 'leftnocase'
                                ? N.toLowerCase().indexOf(G.toLowerCase()) === 0
                                : C === 'right'
                                  ? N.substring(N.length - G.length) === G
                                  : C === 'rightnocase'
                                    ? N.toLowerCase().substring(
                                        N.length - G.length
                                      ) === G.toLowerCase()
                                    : C === 'like'
                                      ? N.indexOf(G) >= 0
                                      : C === 'likenocase'
                                        ? N.toLowerCase().indexOf(
                                            G.toLowerCase()
                                          ) >= 0
                                        : C === Q || C === 'is'
                                          ? N === G
        ...
        y = function() {
          var A = this,
            z = true;
          z =
            x.length === 1 && !x[0].apply(A)
              ? false
              : x.length === 2 && (!x[0].apply(A) || !x[1].apply(A))
                ? false
                : x.length === 3 &&
                  (!x[0].apply(A) || !x[1].apply(A) || !x[2].apply(A))
                  ? false
                  : x.length === 4 &&
                    (!x[0].apply(A) ||
                      !x[1].apply(A) ||
        ...
        insert: function(M, N) {
          ...
          c(J, function(P, Q) {
            ...
            if (T.isArray(P) && Q === 0) {
              c(P, function(S) {
                L.push(
                  z.forcePropertyCase === 'lower'
                    ? S.toLowerCase()
                    : z.forcePropertyCase === 'upper' ? S.toUpperCase() : S
                );
              });
              return true;
            } else {
            ...
            D++;
            P.___id =
              'T' + String(n + b).slice(-6) + 'R' + String(n + D).slice(-6);
            P.___s = true;
            K.push(P.___id);
            if (z.template) {
        ...
        K = TAFFY.mergeObj(TAFFY.mergeObj(r, { insert: undefined }), {
          ...
          context: function(L) {
            if (L) {
              J = TAFFY.mergeObj(
                J,
                L.hasOwnProperty('results')
                  ? TAFFY.mergeObj(L, { run: new Date(), sort: new Date() })
                  : L
              );
            }
            return J;
    ...
    TAFFY.isSameArray = function(y, x) {
      return TAFFY.isArray(y) && TAFFY.isArray(x) && y.join(',') === x.join(',')
        ? true
        : false;
    };
          ...
          u(A, function(B, C) {
            if (
              !(
                (x.isObject(A[C]) &&
                  x.isObject(y[C]) &&
                  x.isSameObject(A[C], y[C])) ||
                (x.isArray(A[C]) &&
                  x.isArray(y[C]) &&
                  x.isSameArray(A[C], y[C])) ||
                A[C] === y[C]
              )
            ) {
              z = false;
